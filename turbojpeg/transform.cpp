...
py::bytes decompress(py::bytes img, TJPF pixelformat, bool fastupsample, bool fastdct) {
	...
	int result;
	int width, height, subsamp, colorspace, precision, progressive, lossless, xdensity, ydensity, densityunits;
...
py::bytes transform(py::bytes img, TJXOP op, int x, int y, int w, int h,
	bool perfect, bool trim, bool crop, bool gray, bool nooutput,
	bool progressive, bool copynone, bool arithmetic, bool optimize) {
	...
	result = tj3Transform(tjInstance, jpegBuf, jpegSize, n, dstBufs, dstSizes, transforms);
	if (result != 0) {
...
PYBIND11_MODULE(turbojpeg, m) {
	...
	m.def("decompress",
		...
		"Decompress JPEG",
		py::arg("img"),
		py::arg("pixelformat") = TJPF_RGB,
		py::arg("fastupsample") = false,
		py::arg("fastdct") = false
	);

	m.def("transform",
		&transform,
		"Losslessly transform JPEG",
		py::arg("img"),
		py::arg("op") = TJXOP_NONE,
		py::arg("x") = -1,
		py::arg("y") = -1,
		py::arg("w") = -1,
		py::arg("h") = -1,
		...
		py::arg("crop") = false,
		py::arg("gray") = false,
		py::arg("nooutput") = false,
		py::arg("progressive") = false,
		py::arg("copynone") = false,
